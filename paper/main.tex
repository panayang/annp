...
$16$ & $\mathbf{43.07\%}$ & $12.50\%$ & $+30.57$pp \\
\bottomrule
\end{tabular}
\caption{Mode A, three seeds at eight regimes and one at sixteen. The
monolithic control sits at the level of a linear baseline without sparse coding
($15.30\%$), confirming the task is not solvable by predicting frequent
targets. Baselines at $32$ and $64$ regimes are still running.}
\end{table}
...
\paragraph{Compose across stages rather than re-walking.}
Multi-hop currently re-applies the same content key at each hop, which is why
paths cost accuracy on a task where no answer requires a path. A second stage
addressed on the \emph{output} of the first would compose rather than repeat,
and would need a source where an answer follows from chaining two stored
relations. Both the variant and the source are absent here.
