...
comparison is decided by the evaluation rather than by the code. A mixed-gap
protocol, cycling horizons within a run, is implemented but not yet run, and
until it is, these numbers bound the mechanism from below rather than
characterising it.
...
an earlier version of this comparison was taken while regime separation had
collapsed; the present one uses a single source, one scale, and a lookup task
